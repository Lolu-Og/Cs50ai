...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        # Get nearby cells
        nearby = self.neighbour(cell)

        # Remove safe cells and moves made
        nearby -= self.safes | self.moves_made

        sentenceupdate = Sentence(nearby, count)

        self.knowledge.append(sentenceupdate)
        ...
        for sentence in self.knowledge:
            if len(sentence.cells) == 0:
                ...
                updatedsafes = sentence.known_safes()
                updatedmines = sentence.known_mines()

                if type(updatedsafes) is set:
                    new_safes |= updatedsafes

                if type(updatedmines) is set:
                    new_mines |= updatedmines
